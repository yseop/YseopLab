...
def get_tokens_sequence(text, token_index):
    ...
    if len(positions) == 0:
        ...
        positions = [el for s in positions for el in s]
        return (positions)
        ...
        stable = []
        for c in range(len(elements) - 1):
            range_ = list(range(elements[c + 1], elements[c + 1] + 3))
            #print(elements[c + 1], elements[c + 1] + 3, c, elements[c], range_, elements[c] + 1)
            if (elements[c] + 1) in range_:
                #print('match +1')
                stable.append(elements[c])
            elif (elements[c] + 2) in range_:
                #print('match+2')
                stable.append(elements[c])
            elif (elements[c] + 3) in range_:
                #print('math +3')
                stable.append(elements[c])
            elif elements[-1]:
                stable.append(elements[-1])
        positions = sorted(stable)
        #positions = [[w] for w in stable]
    #seqs = _get_sequences(*stable)
    ...
    #return max(stable, key=len)
        #print('positions at ', positions)
    return positions
